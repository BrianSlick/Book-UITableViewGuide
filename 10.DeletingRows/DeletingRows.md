...
```
Run the app, and try to swipe-to-delete. This actually can be done in the Simulator, but you may find it easier to test on a device. Note that this point, nothing happens. You either select the row, or nothing happens at all.
...
```
> Comment: I have never had a reason to use the "None" case. I'm honestly don't even know when or why that condition would happen. The "Insert" case is equally rare. So 99.99% of the time, you're only going to care about the "Delete" case.
...
This is wordy, but UITableView really does have some of the best error messages in all of UIKit.
