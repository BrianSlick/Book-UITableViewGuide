...
Between these 4 methods, you can have selection behaviors of "None", "Single", and "Multiple", and then editing mode has its own options. So you could, for example, have multiple selection in edit mode with single selection in regular mode.

These values can be toggled in Storyboard/IB here:
...
We'll continue on with our trust color list, so make your code look like this:
...
First of all, in normal mode, notice how single selection works. You select a row and it stays selected. (We've seen this before, and here we did not include the code to deselect the row). You select another row, and the old one is unselected, leaving you with only one row selected.
...
Yes, this was a long way to go simply to illustrate multiple section. But I felt it was important to highlight a few points:
